...
pub fn generate_lock_file(
    ...
    //default text taht appears top of the lock file
    let default_lock_warning = format!(
    ...
    //check if lock file is empty and append deafut warning first
    let file_size = std::fs::metadata(path_name)
        .expect("file metadata not found")
        .len();
    // println!("file size is:{}", file_size);
    // //proceed to writing dependencies
    if file_size < 4 {
        write!(file, "{}", default_lock_warning)?;
    }
